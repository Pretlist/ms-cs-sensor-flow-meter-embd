...
#
# COLLECTION_BOOST_REQUIRED_COMPONENTS : Boost components that are required
# for Boost_FOUND to be true. The linkage (shared or static) must be
# correct and the library must also be found.
#
# COLLECTION_BOOST_OPTIONAL_COMPONENTS : Boost components that are
...
# variables to be set.
#
# NOTE: If neither COLLECTION_BOOST_REQUIRED_COMPONENTS and
# COLLECTION_BOOST_OPTIONAL_COMPONENTS are defined and/or are empty, then
...
#
# COLLECTION_BOOST_REQUIRED : Whether to set Boost required components as
...
if(NOT COLLECTION_BOOST_REQUIRED)
    # COLLECTION_BOOST_REQUIRED is not set; use the default
    set(COLLECTION_BOOST_REQUIRED "REQUIRED")
elseif(COLLECTION_BOOST_REQUIRED)
    # COLLECTION_BOOST_REQUIRED is set to TRUE/ON/1
    set(COLLECTION_BOOST_REQUIRED "REQUIRED")
else()
    # COLLECTION_BOOST_REQUIRED is not set to TRUE/ON/1
    unset(COLLECTION_BOOST_REQUIRED)
endif()
...
list(LENGTH COLLECTION_BOOST_OPTIONAL_COMPONENTS COLLECTION_BOOST_OPTIONAL_COMPONENTS_LEN)
list(LENGTH COLLECTION_BOOST_REQUIRED_COMPONENTS COLLECTION_BOOST_REQUIRED_COMPONENTS_LEN)
if(COLLECTION_BOOST_OPTIONAL_COMPONENTS_LEN EQUAL 0 AND
   COLLECTION_BOOST_REQUIRED_COMPONENTS_LEN EQUAL 0)
    # just see if Boost can be found
    find_package(Boost ${BOOST_MIN_VERSION} QUIET ${COLLECTION_BOOST_REQUIRED})
    if(Boost_FOUND)
...
if(MINGW)
    list(FIND COLLECTION_BOOST_REQUIRED_COMPONENTS "thread" THREAD_NDX)
    if(NOT ${THREAD_NDX} EQUAL -1)
        list(REMOVE_AT COLLECTION_BOOST_REQUIRED_COMPONENTS ${THREAD_NDX})
        list(INSERT COLLECTION_BOOST_REQUIRED_COMPONENTS ${THREAD_NDX} thread_win32)
    endif()
endif()

# if 'system' is in the list, make sure it comes last; sometimes this
# linkage makes a difference
list(FIND COLLECTION_BOOST_REQUIRED_COMPONENTS "system" SYSTEM_NDX)
if(NOT ${SYSTEM_NDX} EQUAL -1)
    list(REMOVE_AT COLLECTION_BOOST_REQUIRED_COMPONENTS ${SYSTEM_NDX})
    list(APPEND COLLECTION_BOOST_REQUIRED_COMPONENTS "system")
endif()
...
if(MSVC)
    if(VCPKG_TARGET_TRIPLET)
        ...
        if(BOOST_ALL_DYN_LINK)
            ...
            set(COLLECTION_BOOST_REQUIRED_COMPONENTS) #empty components list for static link
...
# if no CXX STANDARD is set, default to that required by COLLECTION: c++14
...
if(${COLLECTION_BOOST_OPTIONAL_COMPONENTS_LEN} GREATER 0)
    message(STATUS "  Looking for optional Boost components...")
    find_package(Boost ${BOOST_MIN_VERSION} QUIET
        OPTIONAL_COMPONENTS ${COLLECTION_BOOST_OPTIONAL_COMPONENTS})
endif()

if(${COLLECTION_BOOST_REQUIRED_COMPONENTS_LEN} GREATER 0)
    message(STATUS "  Looking for required Boost components...")
    find_package(Boost ${BOOST_MIN_VERSION} QUIET
        COMPONENTS ${COLLECTION_BOOST_REQUIRED_COMPONENTS} ${COLLECTION_BOOST_REQUIRED})
endif()
...
if(NOT Boost_FOUND)
    ...
    if(Boost_FOUND AND (${Boost_VERSION} VERSION_EQUAL 1.70.0))
        ...
        if(NOT Boost_LIBRARIES OR "${Boost_LIBRARIES}" STREQUAL "")
            set(Boost_LIBRARIES "")
            foreach(dep IN LISTS COLLECTION_BOOST_REQUIRED_COMPONENTS COLLECTION_BOOST_OPTIONAL_COMPONENTS)
